...
Each reminder screen is backed by a state section in `_agents/state.json`. Automations write these sections; the UI reads them reactively.
...
1. **Mount**: Read cached state from `_agents/state.json` — display immediately (no empty state).
