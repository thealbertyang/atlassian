...
| File | Owner | Purpose |
| --- | --- | --- |
| `_agents/app.config.toml` | user | Workspace preferences (no secrets) |
| `_agents/app-global-state.json` | extension (generated) | Live, read-only cache of effective app state/config/runtime (secrets redacted) |
| `_agents/state.json` | extension (generated) | Persisted runtime state (small, machine-written; no secrets) |

Rules:

- Never treat `_agents/app-global-state.json` as authoritative input. It is a debug artifact for humans/agents.
...
| --- | --- | --- |
| `_agents/config.local.toml` | deprecated | `config/universal.toml` + `_agents/app.config.toml` + `_agents/app-global-state.json` |
| `_agents/config.schema.json` | deprecated | `config/app.schema.json` (engine) + the typed universal contract |

If you need a single "snapshot" artifact, prefer `_agents/app-global-state.json` (generated) over hand-maintained full-state configs.
...
| --- | --- | --- | --- |
| `_agents/events.db` | SQLite | Production | Indexed, queryable event log |
| `_agents/events.jsonl` | Append-only JSONL | Development | Inspectable, git-diffable |
