...
function render() {
  ...
  root.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="row">
        ...
        <input id="baseUrl" type="text" placeholder="https://your-domain.atlassian.net" value="${escapeHtml(
          state.baseUrl,
        )}" />
      </div>
      <div class="row">
        <label for="email">Atlassian account email</label>
        <input id="email" type="email" placeholder="you@company.com" value="${escapeHtml(
          state.email,
        )}" />
      </div>
      <div class="row">
  ...
  saveButton?.addEventListener("click", () => {
    const baseUrl = (document.getElementById("baseUrl") as HTMLInputElement | null)?.value.trim();
    const email = (document.getElementById("email") as HTMLInputElement | null)?.value.trim();
    const apiToken = (document.getElementById("apiToken") as HTMLInputElement | null)?.value.trim();
...
window.addEventListener("message", (event) => {
  const data = event.data as { type?: string; payload?: WebviewState };
